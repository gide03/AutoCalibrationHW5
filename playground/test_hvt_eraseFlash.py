...
TestList = (
    ...
    TestId(b'?65\r', 'Chip Erase External Flash', True),
    TestId(b'?30\r', 'SWITCH TO MAIN FW', True),
)


@click.command()
@click.option('--comport', prompt='Enter com port')
def main(comport):
    #
    # Logger setup    
    #
    filename = f'{CURRENT_PATH}/logs/eraseflash.log'
    print(f'Please find log file at: {filename}')
    ...
    for test in TestList:
        ...
        if test.isRunTest:
            ...
            for i in range(retryAttemp):
                result = transaction(test.id, 180)
